...
- **Existing-board continuation** for adding missing placement, routing, vias,
  ...
  Bind the exact revision, require current handoff/netlist synchronization,
  baseline existing geometry and DRC, then continue from the first incomplete
  dependency gate.
...
- the requirements baseline and every unresolved assumption;
...
The gate closes only when missing information is either resolved or explicitly
accepted as a downstream assumption that does not block PCB architecture. An
...
critical-net intent, or fixed mechanics makes the earlier handoff stale.
