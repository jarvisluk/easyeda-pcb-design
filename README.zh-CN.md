...
生产布线与破坏性修复还必须具备 schema-2 带时间戳操作日志、状态为 `CONTINUE` 的执行预算检查、证明原生板材边界包含关系的 schema-3 布局报告，以及通过独立探针恢复验证的原生 `.epro` 检查点。布线和修复用 JSON 计划交给共享事务执行器；只有保存、重新打开后的当前状态及重复详细 DRC 验证通过，才能推进 gate。
